...
class IronSourceMediationPlugin :
  ...
  override fun onAttachedToEngine(@NonNull flutterPluginBinding: FlutterPluginBinding) {
    ...
    if (!isPluginAttached) {
      ...
      val nativeAdViewFactory =
              LevelPlayNativeAdViewFactoryTemplate(pluginBinding!!.binaryMessenger)
  ...
  override fun onDetachedFromActivityForConfigChanges() {
    Log.d(
            "IRONSOURCE_DEBUG",
            "onDetachedFromActivityForConfigChanges: Thread: ${Thread.currentThread().getName()}"
    )
    Log.d(
            "IRONSOURCE_DEBUG",
            "onDetachedFromActivityForConfigChanges: activity: ${activity?.toString()}"
    )
    if (activity is FlutterActivity) {
  ...
  override fun onReattachedToActivityForConfigChanges(binding: ActivityPluginBinding) {
    Log.d(
            "IRONSOURCE_DEBUG",
            "onReattachedToActivityForConfigChanges: activity: ${activity?.toString()}"
    )
    if (activity is FlutterActivity) {
